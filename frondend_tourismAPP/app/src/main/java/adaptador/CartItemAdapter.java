...
/**
 * Created by YURI VLADIMIR on 03/02/2017.
 */
...
public class CartItemAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        tvName.setText(cartItem.getProduct().getNombre()[Auxiliar.getIdioma()]);
        tvUnitPrice.setText(""+String.valueOf(getPrecio(cartItem)));
        tvQuantity.setText(String.valueOf(cartItem.getQuantity()));
        tvPrice.setText(""+String.valueOf(cart.getCost(cartItem.getProduct())));
        return convertView;
    }
    public double getPrecio(CartItem cart)
    {   double total=0;
        if(cart.getQuantity()==1){total=cart.getProduct().getPrecios()[0];}
        if(cart.getQuantity()==2){total=cart.getProduct().getPrecios()[1];}
        if(cart.getQuantity()==3){total=cart.getProduct().getPrecios()[2];}
        if(cart.getQuantity()==4){total=cart.getProduct().getPrecios()[3];}
        if(cart.getQuantity()>=5){total=cart.getProduct().getPrecios()[4];}
        return total;
    }
