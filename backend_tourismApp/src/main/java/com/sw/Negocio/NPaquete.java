...
//parametros de la base de datos
    ...
    public ArrayList<Paquete> ListarPaquete(int mostrado, int pag) throws SQLException {
        ...
        sql = "select cpaquetecod as cod,\n"
                ...
                + "                cfoto1, cfoto2, cfoto3, cfoto4, cfoto5 from tpaquete where bestado='TRUE' order by cpaquetecod ASC limit(" + (mostrado) + ");";
        ...
        if (pag == 0) {
            while (rs.next()) {
                ...
                paquete.setIdDrawable(ImagenPaquete(rs.getString(14), rs.getString(15), rs.getString(16), rs.getString(17), rs.getString(18)));
                listaDePaquetes.add(paquete);
            }
        } else {
            int contador = 1;
            while (rs.next()) {
                if (mostrado - pag < contador) {
                    Paquete paquete = new Paquete();
                    paquete.setId(rs.getString(1));
                    //paquete.setDescripcion(ProcesarDescripcion(rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6)));
                    paquete.setNombre(ProcesarNombre(rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6)));
    ...
    public String ImagenPaquete(String i1, String i2, String i3, String i4, String i5) {
        if (!ParserRuta(i1).equals("destinoxdefecto.png")) {
            return i1;
        } else {
            if (!ParserRuta(i2).equals("destinoxdefecto.png")) {
                return i2;
            } else {
                if (!ParserRuta(i3).equals("destinoxdefecto.png")) {
                    return i3;
                } else {
                    if (!ParserRuta(i4).equals("destinoxdefecto.png")) {
                        return i4;
                    } else {
                        if (!ParserRuta(i5).equals("destinoxdefecto.png")) {
                            return i5;
                        }
                    }
                }
